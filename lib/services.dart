...
Future<List<int>> readCharacteristic() async {
  ...
  BluetoothCharacteristic c = (await device.discoverServices())[0].characteristics[0];
  return await device.readCharacteristic(c);
}

writeCharacteristic(List<int> value) async {
  if((await device.state) == BluetoothDeviceState.disconnected){
    print('device not connected');
    return;
  }

  var service = (await device.discoverServices())[2];
  var c = service.characteristics[1];
...
class _FlutterBlueAppState extends State<FlutterBlueApp> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: new AppBar(
        title: const Text('Conexão com Bluetooth'),
